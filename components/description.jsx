...
const Description = ({ scrollYProgress, pink }) => {
  const showContent = scrollYProgress.to(s => (s > coordinates.block3.coordinates[0] && s < coordinates.block4.coordinates[1]));
  ...
  return (
    <animated.section className="bg-lavender grid items-center w-full">
      ...
      <animated.div
        style={{
          display: showContent.to(value => value ? 'block' : 'none')
        }}
      ...
      <animated.a
        onClick={handleClick}
        style={{
          display: showContent.to(value => value ? 'flex' : 'none'),
          zIndex: 800
        }}
